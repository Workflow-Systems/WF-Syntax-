...
В Workflow Forms v2 используется пакетная загрузка запросов. Все [`<DataConnection>`](dc_refresh_command.md#data_connections_data_connection) добавляются в очередь загрузки, при этом очередь может подразделяется на несколько подочередей - для асинхронных и синхронных запросов.
...
В Workflow Forms v3 используется параллельная загрузка соединений в рамках одного пакета. Для того чтобы включить этот механизм нужно в тэге `<Parallel>` указать значение True. Если не указывать этот тэг, значение по умолчанию будет False.
