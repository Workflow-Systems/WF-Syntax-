...
description: >-
  ...
  вместо переменных подставляются определенные значения.
---
...
* mode - необязательный параметр типа размера; в качестве значения MODE ожидается [тип размера изображения](export_to_pdf_command.md#image_mode_types) (по умолчанию "Zoom").
...
### ExportBy <a href="#export_by" id="export_by"></a>

Признак определяющий способ формирования pdf-файла.

Необязательный тэг. Значение тэга не ожидается.

Для обязательного тэга `<Type>` ожидается одно из доступных значений:

<table data-header-hidden><thead><tr><th width="212" align="center"></th><th></th></tr></thead><tbody><tr><td align="center">OpenXml</td><td>Для экспорта данных в pdf-файл используется библиотека, не привязанная к приложениям Microsoft Office или Libre Office.</td></tr><tr><td align="center">MsOffice</td><td>Для экспорта данных в pdf-файл используется приложение Microsoft Office, которое должно быть установлено на машине клиента.</td></tr><tr><td align="center">LibreOffice</td><td>Для экспорта данных в pdf-файл используется приложение LibreOffice, которое должно быть установлено на машине клиента.</td></tr></tbody></table>

По умолчанию используется значение MsOffice.

```xml
<ExportBy>
  <Type>OpenXml</Type>
  <LibreOfficePath>..\LibreOffice\program\soffice.exe</LibreOfficePath>
</ExportBy>
```

#### LibreOfficePath <a href="#libre_office_path" id="libre_office_path"></a>

Путь до файла soffice.exe, установленного экземпляра приложения Libre Office, который можно скачать с официального сайта. Учитывается если выбран способ LibreOffice.
...
Необязательный тэг. Значение тэга `<Parameters>`: список тэгов [`<Parameter>`](export_to_pdf_command.md#parameters_parameter).
...
<table data-header-hidden><thead><tr><th align="center"></th><th width="454.3333333333333"></th></tr></thead><tbody><tr><td align="center">Name</td><td><p>Название параметра.</p><p></p><p>Обязательный атрибут. Значение атрибута <code>Name</code>: название одной из <a href="export_to_pdf_command.md#supported_variables">переменных</a>, использующихся в файле шаблона.</p><p></p><p>Переменные в шаблоне задаются как &#x3C;#ParameterName1#> - для отображения скалярного значения или &#x3C;%ParameterName1%> - для отображения массива при использовании в таблице.</p></td></tr></tbody></table>
